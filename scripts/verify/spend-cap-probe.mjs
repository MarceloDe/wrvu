...
for (let i = 1; i <= 20 && !exhausted; i += 1) {
  ...
  log(`edge  ${String(i).padStart(2)}  status=${res.status} allowed=${json.allowed} remaining=${json.remaining} code=${json.code || ""}`);
...
const json = await res.json().catch(() => ({}));
log(`pwa   status=${res.status} code=${json.code || ""} correlationId=${json.correlationId || ""}`);

const pass = exhausted && res.status === 429 && json.code === "daily_cap_reached" && !!json.correlationId;
log(
  pass
