...
// A 1x1 PNG. The point of the probe is the cached SYSTEM prefix, not the image.
const PNG =
  "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==";
const body = JSON.stringify({ template: "ocr", params: {}, images: [PNG] });
...
for (const attempt of [1, 2]) {
  ...
  if (!res.ok) {
    log(`call ${attempt}  status=${res.status} code=${json.code || ""} correlationId=${json.correlationId || ""}`);
