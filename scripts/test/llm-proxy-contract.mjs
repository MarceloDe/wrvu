...
//     are pure functions.
//  2. LIVE layer — the rejection probes over real HTTP against a running server
//     with a real Clerk session. Runs only when PREVIEW and AUTH are set;
...
check("an unknown param is rejected", () =>
  rejects({ template: "ocr", params: { system: "x" }, attachments: [img()] }, "unknown_param"));

check("an unknown top-level field is rejected", () =>
  rejects({ template: "ocr", params: {}, attachments: [img()], extra: 1 }, "unknown_field"));

check("the old `images` wire field is rejected, not silently ignored", () =>
  rejects({ template: "ocr", params: {}, images: ["data:image/png;base64,iVBORw0KGgo="] }, "unknown_field"));

check("a bare data-URL string attachment is rejected", () =>
  rejects({ template: "ocr", params: {}, attachments: ["data:image/png;base64,iVBORw0KGgo="] }, "invalid_attachment"));

check("an attachment carrying extra keys is rejected", () =>
  rejects(
    { template: "ocr", params: {}, attachments: [{ ...img(), cache_control: { type: "ephemeral" } }] },
    "invalid_attachment",
  ));

check("an attachment with a non-base64 source type is rejected", () =>
  rejects(
    { template: "ocr", params: {}, attachments: [{ type: "image", source: { type: "url", media_type: "image/png", data: "x" } }] },
    "invalid_attachment",
  ));

check("an attachment whose data is not base64 is rejected", () =>
  rejects({ template: "ocr", params: {}, attachments: [img("image/png", "data:image/png;base64,AAAA")] }, "invalid_attachment"));

check("a disallowed attachment media type is rejected", () =>
  rejects({ template: "ocr", params: {}, attachments: [img("text/html", "PGI+")] }, "unsupported_media_type"));

check("an image smuggled inside a document block is rejected", () =>
  rejects(
    { template: "timeline", params: {}, attachments: [{ type: "document", source: { type: "base64", media_type: "image/png", data: "iVBORw0KGgo=" } }] },
    "unsupported_media_type",
  ));

check("a PDF is rejected for the ocr template", () =>
  rejects({ template: "ocr", params: {}, attachments: [pdf()] }, "unsupported_media_type"));

check("more attachments than the template allows is rejected", () =>
  rejects({ template: "ocr", params: {}, attachments: Array(9).fill(img()) }, "too_many_attachments"));

check("an oversized attachment is rejected", () =>
  rejects({ template: "ocr", params: {}, attachments: [img("image/png", "A".repeat(9 * 1024 * 1024))] }, "attachment_too_large"));

check("a valid ocr request is accepted", () => {
  const r = parseProxyRequest({ template: "ocr", params: {}, attachments: [img()] });
...
check("a valid timeline request accepts a PDF", () => {
  const r = parseProxyRequest({ template: "timeline", params: {}, attachments: [pdf()] });
  assert(r.ok === true, `expected accept, got ${r.code}`);
  return "accepted";
});
...
check("the user text upstream is the registry's, never the client's", () => {
  const r = parseProxyRequest({ template: "ocr", params: {}, attachments: [img()] });
  const body = buildAnthropicRequest(r.template, r.attachments, "claude-sonnet-4-6");
  const content = body.messages[0].content;
  const text = content[content.length - 1];
  assert(text.type === "text", "the last block is not the user text");
  assert(text.text === PROMPT_TEMPLATES.ocr.userText, "the user text did not come from the registry");
  return `${text.text.length} chars from the registry`;
});
