...
export function buildTimeline(baseline, log, settings) {
  ...
  for (const s of log) {
    ...
    tracked[k] = tracked[k] || { wrvu: 0, studies: 0, um: 0, jhs: 0, byInst: {} };
    for (const i of s.items) {
      const w = i.count * i.wrvu, site = classify(i.inst);
      tracked[k].wrvu += w; tracked[k].studies += i.count;
      tracked[k].byInst[site] = (tracked[k].byInst[site] || 0) + w;
      // um/jhs stay for the two legacy call sites; byInst is the general form.
  ...
  const months = keys.map(k => {
    ...
    return {
      ...
      // Flat `rep_<KEY>` / `trk_<KEY>` keys as well as the nested map: a stacked chart
      // needs one dataKey per series, and it cannot reach into a nested object.
      ...Object.fromEntries(institutions.map((i) => [`rep_${i.key}`, Math.round(total * (shares[i.key] ?? 0))])),
      ...Object.fromEntries(institutions.map((i) => [`trk_${i.key}`, Math.round(t?.byInst?.[i.key] ?? 0)])),
