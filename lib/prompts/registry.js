...
// Attachments arrive as `images: string[]`, each a data URL. A PDF monthly
// report is carried the same way (`data:application/pdf;base64,…`) and is turned
// into a `document` block server-side; the wire type stays string[].
...
const ALLOWED_FIELDS = ["template", "params", "images"];
...
function parseAttachment(value, allowedTypes) {
  if (typeof value !== "string") return { ok: false, code: "invalid_attachment" };
  const m = DATA_URL.exec(value);
  if (!m) return { ok: false, code: "invalid_attachment" };
...
export function parseProxyRequest(body) {
  ...
  const rawImages = Object.prototype.hasOwnProperty.call(body, "images") ? body.images : [];
  if (!Array.isArray(rawImages)) return reject("invalid_images");
  const { min = 0, max = 0, mediaTypes = [] } = template.attachments || {};
  if (rawImages.length < min) return reject("missing_attachment");
  if (rawImages.length > max) return reject("too_many_attachments");

  const attachments = [];
  for (const raw of rawImages) {
    const parsed = parseAttachment(raw, mediaTypes);
    if (!parsed.ok) return reject(parsed.code);
    attachments.push({ mediaType: parsed.mediaType, data: parsed.data });
  }
