...
// pg currently treats sslmode=require / prefer / verify-ca as aliases for verify-full,
// and warns on every connection that this will change: in pg v9 they adopt libpq
// semantics, which are WEAKER — no certificate verification. Two costs today. The
// warning goes to stderr, so Vercel files an error-level log for every single
// /api/exams request and buries anything real. And when v9 lands, the connection
// silently downgrades with no diff to review.
//
// Pinning verify-full changes nothing now — it is what pg already does — and keeps the
// strong behaviour when the default moves. Only the deprecated aliases are rewritten;
// a connection string with no sslmode at all is left exactly as it is, because
// asserting TLS on a URL that never asked for it is a different decision.
const DEPRECATED_SSL_ALIASES = new Set(["require", "prefer", "verify-ca"]);

export function pinSslMode(cs) {
  try {
    const u = new URL(cs);
    if (!DEPRECATED_SSL_ALIASES.has(u.searchParams.get("sslmode"))) return cs;
    u.searchParams.set("sslmode", "verify-full");
    return u.toString();
  } catch {
    return cs;   // not a parseable URL; let the driver produce its own error
  }
}
