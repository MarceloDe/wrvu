...
import { withTenant, llmUsage, llmRateBuckets } from "../db/index.js";
...
export async function consumeRateToken(userId) {
  ...
  const rows = await withTenant(userId, ({ db }) => db.execute(sql`
...
export async function checkCap(userId) {
  ...
  const rows = await withTenant(userId, ({ db }) =>
    db.select({ spent: sql`coalesce(sum(${llmUsage.costEstimate}), 0)` })
      .from(llmUsage)
